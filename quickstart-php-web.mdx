...
# No, just use docker compose file
# docker build -t phpimage .
# docker run -it --volume ./your-application:/var/www/html --name phpbox -p 9012:80 phpimage
# docker start -ai phpbox

docker exec -it phpbox /bin/bash
...
RUN sed -i 's|/var/www/html|/var/www/html/public|g' /etc/apache2/apache2.conf
  ...
  php:
    build:
      context: .
      dockerfile: phpimage.Dockerfile
    container_name: phpbox
    ports:
      - 9012:80
    volumes:
      - ./your-application:/var/www/html
    extra_hosts:
...
need to save user details to session

explain CSRF token get /post same origin difference
...
The `--volume .://var/www/html` parameter shares your current folder with the container, so both your physical machine and Docker can read and write the same files.
